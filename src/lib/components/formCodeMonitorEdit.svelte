<script lang="ts">
	import type { monitorHTTPCodeDBType } from '$lib/types';
	...
	export let CodeMonitor: monitorHTTPCodeDBType;
	export let CodeForm;
	...
	$: if (CodeMonitor.req_headers) {
		ParsedHeader = JSON.parse(CodeMonitor.req_headers);
	}
</script>
...
<form action="?/editstandard" method="POST" use:editenhance>
	...
	<div>
		...
		<Input
			name="status_code"
			autocomplete="off"
			bind:value={$editform.status_code}
			aria-describedby={$editerrors.status_code
				? 'status_code-error status_code-desc'
				: 'status_code-desc'}
			aria-invalid={$editerrors.status_code ? 'true' : undefined}
		/>

		<span id="status_code-error" aria-live="assertive" class=" text-destructive text-sm">
			{#if $editerrors.status_code}
				{$editerrors.status_code}
			{/if}
		</span>
	</div>
	...
	<div class="my-2">
		<div class="flex flex-row justify-between">
			...
			<Switch
				checked={CodeMonitor.follow_redir}
				class="mt-auto"
				onCheckedChange={() => {
	...
	<div class="my-2">
		<div class="flex flex-row justify-between">
			...
			<Switch
				checked={CodeMonitor.ssl_verify}
				class="mt-auto"
				onCheckedChange={() => {
	...
	<div class="my-2">
		...
		>
			...
			<Select.Trigger type="button" class="w-full">
				<Select.Value placeholder={CodeMonitor.interval} />
			</Select.Trigger>
			<Select.Content>
	...
	<div class="my-2">
		...
		>
			...
			<Select.Trigger type="button" class="w-full">
				<Select.Value placeholder={CodeMonitor.req_timeout} />
			</Select.Trigger>
	...
	<div class="my-2">
		...
		>
			...
			<Select.Trigger type="button" class="w-full">
				<Select.Value placeholder={CodeMonitor.method.toUpperCase()} />
			</Select.Trigger>
	...
	<div class="my-2">
		<div>
			...
			>
				...
				<Select.Trigger type="button" class="w-full">
					<Select.Value placeholder={CodeMonitor.checks_up} />
				</Select.Trigger>
				<Select.Content>
	...
	<div>
		<div>
			...
			>
				...
				<Select.Trigger type="button" class="w-full">
					<Select.Value placeholder={CodeMonitor.checks_down} />
				</Select.Trigger>
				<Select.Content>
					<Select.Item value={1}>1</Select.Item>
					<Select.Item value={2}>2</Select.Item>
