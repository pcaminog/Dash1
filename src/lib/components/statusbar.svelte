	...
	<div class="md:flex">
		<div class="flex my-2 md:my-0 mr-0.5">
			{#each Array(50).fill(null) as _, i}
				{#if monitorData[i] && monitorData[i].ok}
					<Tooltip.Root>
						<Tooltip.Trigger
							><div
								class="h-6 w-2 inline-block bg-green-600 rounded-md mr-0.5 min-w-min"
							/></Tooltip.Trigger
						>
						<Tooltip.Content>
				...
				{:else if monitorData[i] && !monitorData[i].ok}
					<Tooltip.Root>
						<Tooltip.Trigger
							><div
								class="h-6 w-2 inline-block bg-destructive rounded-md mr-0.5"
								style="min-width: 7px;"
							/></Tooltip.Trigger
						>
						<Tooltip.Content>
				...
				{:else}
					<div
						class="h-6 w-2 inline-block border border-black rounded-md mr-0.5"
						style="min-width: 7px;"
					/>
				{/if}
			{/each}
