			...
			{#each Array(50).fill(null) as _, i}
				{#if monitorData[i] && monitorData[i].status >= 200 && monitorData[i].status <= 299}
					<Tooltip.Root>
						<Tooltip.Trigger
							><div
								class="h-6 md:w-3 w-1 inline-block bg-green-600 rounded-md mr-0.5 min-w-min"
							/></Tooltip.Trigger
						>
				...
				{:else if monitorData[i] && (monitorData[i].status < 200 || monitorData[i].status > 299)}
					<Tooltip.Root>
						<Tooltip.Trigger
							><div
								class="h-6 md:w-3 w-1 inline-block bg-destructive rounded-md mr-0.5"
				...
				{:else}
					<div
						class="h-5 md:w-3 w-1 inline-block border border-black rounded-md mr-0.5"
						style="min-width: 10px;"
					/>
				{/if}
			{/each}
