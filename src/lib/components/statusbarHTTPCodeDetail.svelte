<script lang="ts">
	...
	export let expectedStatus: number;
</script>
	...
	<div class="md:flex">
		<div class="flex my-2 md:my-0 mr-0.5">
			{#each Array(50).fill(null) as _, i}
				{#if monitorData[i] && monitorData[i].status === expectedStatus}
					<Tooltip.Root>
						<Tooltip.Trigger
							><div
				...
				{:else if monitorData[i] && monitorData[i].status !== expectedStatus}
					<Tooltip.Root>
						<Tooltip.Trigger
							><div
